...
chomp($arch);
my $reldir =($basedir."/../releases/spsim-$version-$arch");
my $builddir = ($basedir."/../releases/spsim-build-$arch");
`rm -rf $reldir`;
`mkdir -p $reldir`;
`mkdir -p $builddir`;
...
unless(`uname -s` =~ /MINGW32/){
	...
	unless($dep =~ /Qt/ || $dep =~ /libtiff/ ||  $dep =~ /libpng/ || $dep =~ /libgsl/ || 
	       ...
	       $dep =~ /libcufft/ || $dep =~ /libcuda/ || $dep =~ /libcudart/ || $dep =~ /libtlshook/ || $dep =~ /libnfft/ || $dep =~ /CUDA/){
	    next;
	}
	if(-f $dep){
#	  if($dep =~ /(.*\.framework)/){
...
chdir("..");
#if(`uname -s` =~ /Darwin/){
#  system("hdiutil create -srcfolder spsim-$version-$arch -volname spsim-$version spsim-$version.dmg");
#}else{
#  system("tar -zcvf ".$reldir.".tar.gz spsim-$version-$arch");
#}
